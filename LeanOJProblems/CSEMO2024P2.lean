...
def IsJChord (P : Set (EuclideanSpace ℝ (Fin 2))) (A B : EuclideanSpace ℝ (Fin 2)) : Prop :=
  A ∈ frontier P ∧ B ∈ frontier P ∧ A ≠ B ∧ ∀ X ∈ P, 0 ≤ inner ℝ (X-A) (B-A) ∧
    0 ≤ inner ℝ (X-B) (A-B)
...
theorem solution :
    IsGreatest {k : ℕ | k > 0 ∧ (∀ P : Set (EuclideanSpace ℝ (Fin 2)), IsConvexPolygon P →
      HasAtLeastKJChords P k)} answer := by sorry
